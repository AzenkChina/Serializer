...
//在Lua Code中注册的enum,为了避免冲突,以名称作为enumTable来存储
...
LUALIB_API int luaopen_power(lua_State *L)
{
	...
	LUA_ENUM(L, Reactive);
	LUA_ENUM(L, Apparent);
	lua_setglobal(L,"__Quadrant");
	
	//注册Voltage枚举
	lua_newtable(L);
	LUA_ENUM(L, Normal);
	LUA_ENUM(L, Abnormal);
	lua_setglobal(L,"__Voltage");
